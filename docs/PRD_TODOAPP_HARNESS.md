...
`ARCHITECTURE_ROADMAP.md`. The harness now also builds and executes the same
...
1) **Haxe → TypeScript** compilation (TS + TSX), strict typechecking, and runtime correctness.
   The identical Haxe source must also degrade to direct classic ESM JavaScript
   plus strictly consumable declarations.
...
- Keep **classic Genes** output green both as the compiler baseline and as a
  first-class todoapp profile. Product-specific forks or compiler branches are
  forbidden; both profiles consume `examples/todoapp/src/`.
...
- Keep examples **browsable and auditable**:
  - `src-gen/` is ephemeral build output (gitignored)
  - `dist-ts/src-gen/` is the committed “intended output” snapshot
  - `dist/` is runtime build output (bundled assets / `tsc` output), not the compiler snapshot

---

## Functional requirements — Todoapp (examples/todoapp)

### Layout conventions (for interop clarity)

To keep the interop story obvious and teachable, prefer explicit “source roots”:

- `examples/todoapp/src/` — Haxe sources (the app code)
- `examples/todoapp/{web,server}/src-ts/` — TS/TSX sources authored directly in TypeScript (interop fixtures)
- `examples/todoapp/{web,server}/src-gen/` — generated TS/TSX output (gitignored, regenerated by builds)
- `examples/todoapp/{web,server}/dist-ts/src-gen/` — committed “intended output” snapshots (used for diffs)
- `examples/todoapp/{web,server}/dist/` — runtime build output (bundled assets / `tsc` output)
- `examples/todoapp/{web,server}/classic-src-gen/` — ephemeral direct classic
  ESM plus `.d.ts`; the web bundle lands in `web/classic-dist/`

The acceptance criteria below assumes we adopt (or migrate toward) this structure.
...
## Decisions (initial defaults)

These resolve the “open questions” below for the initial implementation. They can be revisited later.

1) SSR: **no SSR for now** (client-only web app). Keep the harness variables minimal; add SSR later as an optional fixture if we need coverage.
2) “TS imports Haxe output” demo: **server-first** (Node) for simplicity and determinism, then expand to the web bundle once the pattern is proven.
3) Low-level React output mode: keep it as a **second build target inside `examples/todoapp/`** (not a separate example) so it shares shared code and stays easy to compare.
4) ts2hx roundtrip fixture: **Node-only first** (no DOM) to avoid bundler/DOM variability; consider TSX later once baseline roundtrip is stable.

## Open questions (need decisions)

Tracked as “future extensions” (not required for the first pass of this PRD):

1) Add SSR coverage (`ReactDOMServer`) as an opt-in variant/fixture.
2) Add the “TS imports Haxe output” demo to the web bundle as well (proves bundler ergonomics).
3) If the in-place second build target becomes too complex, split into `examples/todoapp-lowlevel/`.
4) Add TSX to the ts2hx roundtrip fixture once Node-only stability is proven.
