...
| Output transaction | `yarn test:output-transaction` | TS and classic implementations, declarations, support files, maps, and exact v2 ownership manifests roll back after pre-publication and real rename failures; punctuation- and extension-distinct entrypoints remain isolated; successful rebuilds remove stale owned files and preserve unrelated assets plus ambiguous legacy manifests | Simultaneous unrelated writers targeting the same final path or cross-process read isolation between individual renames |
