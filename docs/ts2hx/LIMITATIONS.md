...
- switch fallthrough and unlabelled switch-to-enclosing-loop `continue` are
  ...
  is rejected;
- synchronous unlabelled return, break, and continue with an explicit strong
  function return type can cross one or more `finally` callbacks in a named
...
The registered snapshot suite covers 22 projects and currently compares 50
generated files. Eleven projects require `genes-esm`; eleven are request-free
...
- `finally-completion-control`: strict and request-free; it executes local and
