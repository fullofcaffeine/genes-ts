...
| Compiler timing attribution | `src/genes/util/Timer.hx`, `src/genes/Generator.hx`, `src/genes/ts/TsModuleEmitter.hx` | Uses Haxe's `--times` tree for parsing, typing, DCE, macros, and the custom generator. Request-local Genes timers identify planning, validation, emission, and publication. The benchmark-only profile adds nested TypeScript class timers. Ordinary builds compile without these fine timers. |
