...
| TypeScript null-narrowing proof (migration seam) | Currently `src/genes/ts/TsModuleEmitter.hx`; planned `TsNarrowingPlan` | Recognizes branch-local and continuation non-null facts used only to produce honest TypeScript. The [ownership inventory](TS_NARROWING_OWNERSHIP.md) records why invalidation must move out of token emission without becoming a shared CFG or universal IR. |
