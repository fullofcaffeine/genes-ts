...
| GRA-009: public-surface audit enrollment | **Reproduced and remediated P1 evidence gap.** A fail-first fixture showed that a hand-selected safe file could pass while a compiler-owned sibling exported inferred `any`. Production profiles now derive enrollment from exact Genes v2 ownership manifests; every owned type module is audited or receives a stale-detecting documented classification. Recursive generic expansion is bounded by declaration ownership, and pre-existing Dynamic-heavy full/classic surfaces remain honestly tracked by `genes-ofy` rather than being relabeled as safe. | `genes-7dt` |
| GRA-010: symlink/reparse containment | **Reproduced and remediated P1 on POSIX.** With Haxe 4.3.7, a generated package-directory link redirected a successful publication into an external casualty, and a deterministic stage link exposed recursive cleanup to the same class of escape. `OutputTransaction` now compares every existing public/stage component with symlink-resolving `FileSystem.fullPath` before reads or mutation; strict publication fails closed, while abort cleanup skips hostile links. The executable fixture covers macOS and Linux semantics. Windows junction behavior remains a separate platform experiment rather than an inferred claim. | `genes-rby` |
