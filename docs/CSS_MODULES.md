...
keys. Genes validates its finite result; it does not build a second CSS parser
that could disagree with the application.
...
The returned candidate contains the validated manifest, its digest, the
portable companion path, complete Haxe bytes, and the exact Haxe/runtime field
mapping. The function deliberately does not write files. A host can inspect the
candidate and include it in its normal safe publication step.
...
7. pinned esbuild loads and runs both generated programs through a controlled
   CSS Modules loader;
8. the runtime object must contain the three reviewed keys and string values.
...
This first increment is intentionally one-shot:
