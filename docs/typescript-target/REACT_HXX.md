...
Component tags are intentionally narrower than ordinary safe values. Source
JSX may nest an intrinsic tag such as `div`, a fragment, a component already
held in a local variable, or an exact field-level `@:jsRequire` that resolves
directly to one default or named ESM export:

```haxe
private extern class Components {
  @:jsRequire("./Card.js", "default")
  static function Card(props:CardProps):Element;
}

return <Components.Card><span>Ready</span></Components.Card>;
```

That field does not become a JavaScript object-property read. Dependency
planning gives it a local import such as `Card`, and reading an ESM import
binding cannot invoke a getter or Proxy trap. The JSX plan and dependency plan
use the same metadata normalizer, so the movement decision cannot mistake a
dotted property path for a direct import.

All other field-based tags stay explicit, including class-level imports,
dotted imports such as `"Components.Card"`, and static methods generated by
Haxe. Those forms still produce JavaScript property reads, which may be backed
by a getter, native mapping, module wrapper, or Proxy. Keeping the child local
preserves the original `Child,Parent` read order. Code may also read a
component field into a local first when it wants to establish an explicit,
stable lexical value.
...
The focused direct-import fixture separately verifies the narrow ESM case and
its field-read controls across TypeScript 5, 6, and 7:
