...
### Preserve a type Haxe erases before generic emission

Primitive-backed abstracts can disappear even earlier. In this example Haxe
checks `CellPhase`, then represents `Cell<CellPhase>` as `Cell<String>` in the
typed generic call:

```haxe
enum abstract CellPhase(String) to String {
  final Pending = "pending";
  final Ready = "ready";
}

final phase:Cell<CellPhase> =
  Cells.makeCell(CellPhase.Pending);
```

Declaration-only `@:ts.explicitTypeArguments` can no longer recover a type that
is absent from that later typed node, so the unassisted output widens:

```ts
const phase: Cell<string> = makeCell<string>("pending");
```

A typed library macro that still owns the source-level type can preserve it:

```haxe
final phase:Cell<CellPhase> = genes.ts.TypeArguments.call(
  Cells.makeCell(CellPhase.Pending),
  CellPhase.Pending
);
```
