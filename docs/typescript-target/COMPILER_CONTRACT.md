...
- A hidden `.genes-output-<entry>.manifest` records exactly which relative
  paths that entrypoint owns. A later successful build removes stale paths from
  its prior manifest, but never removes unrelated assets or files owned by a
  different entrypoint/tool.
