...
async function compilePair(
  ...
  scenario: Scenario,
  timeoutMs: number,
  inspectResults?: (cold: ProcessResult, warm: ProcessResult) => void
...
/** Returns only this fixture module's stable indexed-plan descriptions. */
function indexedInventory(result: ProcessResult): ReadonlyArray<string> {
  return [...resultText(result).matchAll(
    /\[GTS-INDEX-INVENTORY\] servercase\.Main:\d+:([^\r\n]+)/g
  )].map((match) => match[1]);
}

/**
 * Proves indexed decisions belong to one cold or warm compiler request.
 *
 * The safe request needs an exact concrete-slot assertion. The middle request
 * moves the same operation behind an explicit Dynamic boundary and must choose
 * the direct native operation instead. Returning to the safe request must
 * reproduce both the original decision inventory and generated bytes.
 */
async function assertIndexedAccessServerIsolation(
  compiler: ReturnType<typeof selectedHaxeCompiler>,
  server: OwnedHaxeCompilerServer,
  timeoutMs: number
): Promise<void> {
  const safe: Scenario = {
    id: "a-indexed-plan-server",
    project: "project-a",
    profile: tsProfile,
    defines: [
      "genes.ts.indexed_access_inventory",
      "server_indexed_plan"
    ]
  };
  let safeInventory: ReadonlyArray<string> = [];
  const safeTree = await compilePair(
    compiler,
    server,
    safe,
    timeoutMs,
    (cold, warm) => {
      ...
      deepStrictEqual(
        ...
        safeInventory,
        "warm safe indexed inventory differs from its cold request"
      );
    }
  );
  ...
  const dynamic: Scenario = {
    ...safe,
    defines: [...(safe.defines ?? []), "server_indexed_dynamic"]
  };
  await compilePair(
    compiler,
    server,
    ...
    (cold, warm) => {
      ...
      deepStrictEqual(
        indexedInventory(warm),
        coldInventory,
        "warm Dynamic indexed inventory differs from its cold request"
      );
    }
  );
  ...
  deepStrictEqual(
    await compilePair(
      ...
      timeoutMs,
      (cold, warm) => {
        deepStrictEqual(indexedInventory(cold), safeInventory,
          "restored cold indexed inventory differs from the first request");
        deepStrictEqual(indexedInventory(warm), safeInventory,
          "restored warm indexed inventory differs from the first request");
      }
    ),
    safeTree,
    "safe indexed output changed after a warm Dynamic request"
  );
}
