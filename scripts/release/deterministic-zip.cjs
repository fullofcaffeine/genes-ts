...
// ZIP stores a timezone-free DOS date. This fixed wall-clock value keeps the
// same package bytes across operating systems, time zones, and rebuilds.
...
/**
 * Encode named file bytes using the release archive's one canonical ZIP shape.
 *
 * Release packages are public, immutable supply-chain artifacts. Sorting UTF-8
 * paths and fixing timestamps, modes, compression, and host OS makes a rebuild
 * of one source commit produce the same bytes instead of inheriting incidental
 * filesystem metadata from a CI runner.
 */
...
if (require.main === module) {
  try {
    ...
    if (!source || !output || rest.length > 0) {
      throw new Error(
        "usage: deterministic-zip.cjs <source-directory> <output.zip>"
      );
    }
