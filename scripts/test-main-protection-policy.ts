...
function main(): void {
  ...
  const toolingMinimumNode = genesTs.indexOf("node-version: 22.22.0");
  const toolingMinimumCheck = genesTs.indexOf(
    "- name: Verify the packed tooling package on Node 22.22"
  );
  ...
  assert(
    ...
    "The required genes-ts job must pack tooling, check Node 22.22, and restore the repository Node release"
  );
  assert(
    genesTs.includes('--tarball "$TOOLING_MINIMUM_TARBALL"')
      && genesTs.includes('--pack-json "$TOOLING_MINIMUM_PACK_JSON"'),
    "The Node 22.22 check must inspect the exact packed tooling candidate"
  );
