 ...
 * literal unions and anonymous typedef field literal unions. Public API facts
 * are intentionally owned by the target-neutral `genes.PublicSurface`; keeping
 * them here would mix TypeScript-specific recovery with declaration planning.
 * This cache deliberately does not store arbitrary rendered types, because
 * that would make it a shadow type printer.
 *
