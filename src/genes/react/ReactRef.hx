 ...
 *
 * The outer `Null` models React's explicit `null` ref. `HtmlPropsOf` wraps this
 * alias in `Undefinable` before marking the property optional. That separation
 * matters because Haxe otherwise represents both omission and the authored
 * nullable value with one outer `Null` and the HXX checker could lose the
 * explicit-null contract while normalizing optional fields.
 */
