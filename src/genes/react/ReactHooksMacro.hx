...
private inline final EFFECT_DIAGNOSTIC = "GTS-REACT-EFFECT-001";
...
private function mayBeCallable(type: Type, depth: Int = 0): Bool {
  if (depth > 32)
    return true;
  return switch type {
    case TMono(reference): final resolved = reference.get(); resolved == null || mayBeCallable(resolved,
        depth
        + 1);
    case TLazy(resolve):
...
private function hasUnresolvedBoundary(type: Type, depth: Int = 0): Bool {
  if (depth > 32)
    return true;
  return switch type {
    case TMono(reference): final resolved = reference.get(); resolved == null || hasUnresolvedBoundary(resolved,
        depth
        + 1);
    case TLazy(resolve):
    ...
    case TFun(arguments, result): arguments.exists(argument ->
        hasUnresolvedBoundary(argument.t, depth
        + 1)) || hasUnresolvedBoundary(result, depth + 1);
    case TInst(reference, arguments):
      switch reference.get().kind {
        case KTypeParameter(_): true;
        case _:
          arguments.exists(argument -> hasUnresolvedBoundary(argument,
            depth + 1));
      }
    ...
    case TAnonymous(reference):
      reference.get()
        .fields.exists(field -> hasUnresolvedBoundary(field.type, depth + 1));
  }
}
...
private function isDependencyCallee(expression: Expr): Bool {
  ...
  for (entry in Context.getLocalImports()) {
    ...
    switch entry.mode {
      case INormal
        if (path.join(".") == memberPath.join(".") && localName == "deps"):
        return true;
      case IAsName(alias)
        if (path.join(".") == memberPath.join(".") && localName == alias):
        return true;
      case IAll
        if (path.join(".") == modulePath.join(".") && localName == "deps"):
        return true;
      case _:
    }
  }
...
private function abstractIdentity(type: Type, moduleName: String,
  ...
  if (depth > 32)
    return false;
  return switch type {
    case TMono(reference): final resolved = reference.get(); resolved != null && abstractIdentity(resolved,
        moduleName, typeName, depth
        + 1);
    case TLazy(resolve):
      abstractIdentity(resolve(), moduleName, typeName, depth + 1);
    case TType(reference, arguments):
      final value = reference.get();
      abstractIdentity(applied(value.type, value.params, arguments),
        moduleName, typeName, depth + 1);
...
private function snapshotMemo(calculate: Expr, dependencies: Array<Expr>,
  ...
  for (index in 0...dependencies.length) {
    ...
    if (argument.type != null) {
      ...
      if (!Context.unify(actual,
        expected) || !Context.unify(expected, actual)) {
        fail(SNAPSHOT_DIAGNOSTIC,
...
private function isEmptyArrayLiteral(expression: Expr): Bool {
  return switch expression.expr {
    case EParenthesis(inner) | EMeta(_, inner) | ECheckType(inner, _) |
      ECast(inner, _):
...
private function needsExplicitStateType(type: Type, depth: Int = 0): Bool {
  if (depth > 32)
    return false;
  return switch type {
    case TMono(reference): final resolved = reference.get(); resolved != null && needsExplicitStateType(resolved,
        depth
        + 1);
    case TLazy(resolve):
...
private function checkedDependencies(typeArguments: Array<Expr>,
    emittedArguments: Array<Expr>, position: Position): Expr {
  final elementType = dependencyType(typeArguments, position);
  ...
  final literal: Expr = {
    expr: EArrayDecl(emittedArguments),
    pos: position
  };
  return {
...
private function isVoid(type: Type, depth: Int = 0): Bool {
  if (depth > 32)
    return false;
  return switch type {
    case TMono(reference): final resolved = reference.get(); resolved != null && isVoid(resolved,
        depth
        + 1);
    case TLazy(resolve):
      isVoid(resolve(), depth + 1);
    case TType(reference, arguments):
      final value = reference.get();
      isVoid(applied(value.type, value.params, arguments), depth + 1);
    case TAbstract(reference, _): final value = reference.get(); value.module == "StdTypes" && value.name == "Void";
    case _:
      false;
  }
}

private function isEffectCleanup(type: Type): Bool {
  return switch type {
    case TFun(arguments, result): arguments.length == 0 && isVoid(result);
    case _:
      false;
  }
}

private function withDiscardedBlockResult(effect: Expr): Null<Expr> {
  return switch effect.expr {
    case EParenthesis(inner):
      final rewritten = withDiscardedBlockResult(inner);
      rewritten == null ? null : {
        expr: EParenthesis(rewritten),
        pos: effect.pos
      };
    case EMeta(metadata, inner):
      final rewritten = withDiscardedBlockResult(inner);
      rewritten == null ? null : {
        expr: EMeta(metadata, rewritten),
        pos: effect.pos
      };
    case EFunction(kind, value):
      switch value.expr.expr {
        case EBlock(expressions) if (value.ret == null):
          final returned: Expr = {
            expr: EReturn(null),
            pos: value.expr.pos
          };
          {
            expr: EFunction(kind, {
              ...
              expr: {
                expr: EBlock(expressions.concat([returned])),
                pos: value.expr.pos
              },
              params: value.params
            }),
        ...
        case EMeta(metadata, returned)
          if (metadata.name == ":implicitReturn" && value.ret == null):
          switch returned.expr {
            case EReturn(result) if (result != null):
              switch result.expr {
                case EBlock(expressions):
                  ...
                  };
                  {
                    expr: EFunction(kind, {
                      ...
                      ret: TPath({pack: [], name: "Void"}),
                      expr: {
                        expr: EBlock(expressions.concat([finished])),
                        pos: result.pos
                      },
                      params: value.params
                    }),
...
private function effectResult(effect: Expr): {
  cleanup: Bool,
  expression: Expr
} {
  final discarded = withDiscardedBlockResult(effect);
  if (discarded != null) {
    return {cleanup: false, expression: discarded};
  }
  return switch Context.typeof(effect) {
    case TFun(arguments, result) if (arguments.length == 0):
      if (isVoid(result)) {
        {cleanup: false, expression: effect};
      } else if (isEffectCleanup(result)) {
        {cleanup: true, expression: effect};
      } else {
        fail(EFFECT_DIAGNOSTIC,
          "useEffect callback must return Void or one Void->Void cleanup " +
          "callback.",
          effect.pos);
      }
    case _:
      fail(EFFECT_DIAGNOSTIC, "useEffect requires a zero-argument callback.",
        effect.pos);
  }
}
...
function useState(initial: Expr): Expr {
  ...
  return if (isNullLiteral(initial) || isEmptyArrayLiteral(initial)
    || needsExplicitStateType(type)) {
    macro @:pos(position) genes.ts.TypeArguments.call(genes.react.ReactHookBindings.useStateContextual($initial),
      $initial);
  } else {
    macro @:pos(position)
...
function useMemo(calculate: Expr, dependencies: Expr): Expr {
  ...
  final checked = checkedDependencies(arguments, snapshot.dependencies,
    dependencies.pos);
  final position = Context.currentPos();
...
function useCallback(callback: Expr, dependencies: Expr): Expr {
  final arguments = dependencyArguments(dependencies, "useCallback");
  final checked = checkedDependencies(arguments, arguments, dependencies.pos);
  final position = Context.currentPos();
  return macro @:pos(position)
    genes.react.ReactHookBindings.useCallback($callback, $checked);
}

function useOptimistic(passthrough: Expr, reducer: Expr): Expr {
  final position = Context.currentPos();
  return macro @:pos(position)
    genes.react.ReactHookBindings.useOptimistic($passthrough, $reducer);
}

function createContext(defaultValue: Expr): Expr {
  final position = Context.currentPos();
  return macro @:pos(position)
    genes.react.ReactHookBindings.createContext($defaultValue);
}

function useContext(context: Expr): Expr {
  final position = Context.currentPos();
  return macro @:pos(position)
    genes.react.ReactHookBindings.useContext($context);
}

function useEffect(effect: Expr, dependencies: Expr): Expr {
  final arguments = dependencyArguments(dependencies, "useEffect");
  final checked = checkedDependencies(arguments, arguments, dependencies.pos);
  final result = effectResult(effect);
  final position = Context.currentPos();
  return if (result.cleanup) {
    macro @:pos(position)
    genes.react.ReactHookBindings.useEffectCleanup(${result.expression},
      $checked);
  } else {
    macro @:pos(position)
    genes.react.ReactHookBindings.useEffectVoid(${result.expression}, $checked);
  }
}

function useRef(initial: Expr): Expr {
  final position = Context.currentPos();
  return
    macro @:pos(position) genes.ts.TypeArguments.call(genes.react.ReactHookBindings.useRef($initial),
      $initial);
}
