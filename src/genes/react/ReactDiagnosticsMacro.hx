...
private function fullTypeName(type: ClassType): String {
  ...
  return
    type.module == primaryTypeName ? primaryTypeName : '${type.module}.${type.name}';
}
...
private function mergeFlows(values: Array<ReturnFlow>): ReturnFlow {
  return {
    ...
    alwaysReturns: values.length > 0
    && values.foreach(value -> value.alwaysReturns)
  };
}
...
private function unwrapCallee(expression: TypedExpr): TypedExpr {
  return switch expression.expr {
    case TParenthesis(inner) | TCast(inner,
      _) | TMeta(_, inner): unwrapCallee(inner);
    case _: expression;
  };
}
...
private function callKind(callee: TypedExpr): Null<ReactCallKind> {
  ...
  return switch expression.expr {
    case TField(_, access):
      ...
      } else if (resolved.owner.module == "Math"
        && resolved.field.name == "random") {
        KnownImpure("Math.random");
      } else if (resolved.owner.module == "Date"
        && resolved.field.name == "now") {
...
private function validateHookCall(owner: ClassType, field: ClassField,
    value: ReactPlacement, position: Position): Void {
...
private function validateReactUse(owner: ClassType, field: ClassField,
    value: ReactPlacement, position: Position): Void {
  final label = fieldLabel(owner, field);
  if (!isReactFunction(value)) {
    fail("GTS-REACT-USE-003",
      'React use binding $label may only be called from a @:genes.reactComponent or @:genes.reactHook function.',
      position);
  }
  if (value.nestedFunction) {
    fail("GTS-REACT-USE-003",
...
private function isCurrentDateConstruction(callee: TypedExpr,
  ...
  if (resolved == null || resolved.owner.module != "js.Syntax"
    || resolved.field.name != "construct") {
    return false;
  }
...
private function failImpureCall(label: String, value: ReactPlacement,
...
private function validateCall(call: TypedExpr, callee: TypedExpr,
    arguments: Array<TypedExpr>, value: ReactPlacement): Void {
...
private function validateOrdinaryUseName(callee: TypedExpr,
    value: ReactPlacement, position: Position): Void {
  if (!isReactFunction(value)) {
    return;
  }
...
private function validateConstruction(type: ClassType,
    arguments: Array<TypedExpr>, value: ReactPlacement,
    position: Position): Void {
...
private function validateStaticMutation(target: TypedExpr,
    value: ReactPlacement, position: Position): Void {
  if (!isReactFunction(value) || value.nestedFunction) {
    return;
  }
...
private function analyzeList(expressions: Array<TypedExpr>,
    value: ReactPlacement): ReturnFlow {
...
private function analyzeBlock(expressions: Array<TypedExpr>,
    value: ReactPlacement): ReturnFlow {
  var current = value;
  var mayReturn = false;
...
private function analyze(expression: TypedExpr,
    value: ReactPlacement): ReturnFlow {
  return switch expression.expr {
    ...
    case TBinop(op, left, right): switch op {
        case OpAssign | OpAssignOp(_): validateStaticMutation(left, value,
            expression.pos);
        case _:
        ...
        value).mayReturn || analyze(right, rightPlacement).mayReturn ? {
        mayReturn: true,
        alwaysReturns: false
      } : NO_RETURN;
    ...
    case TParenthesis(inner) | TCast(inner, _) | TMeta(_, inner) |
      TEnumParameter(inner, _, _) | TEnumIndex(inner):
      analyze(inner, value);
    ...
    case TUnop(op, _, inner):
      switch op {
        case OpIncrement | OpDecrement: validateStaticMutation(inner, value,
            expression.pos);
        case _:
      }
      analyze(inner, value);
    ...
    case TFor(_, iterator, body):
      ...
      final flow = mergeFlows([analyze(iterator,
        value), analyze(body, loopPlacement)]);
    ...
    case TIf(condition, positive, negative):
      ...
      final negativeFlow = negative == null ? NO_RETURN : analyze(negative,
        branchPlacement);
      {
        mayReturn: conditionFlow.mayReturn || positiveFlow.mayReturn
        || negativeFlow.mayReturn,
        alwaysReturns: conditionFlow.alwaysReturns
        || (negative != null && positiveFlow.alwaysReturns
          && negativeFlow.alwaysReturns)
      };
    case TWhile(condition, body, _):
      final loopPlacement = withLoop(value);
      final flow = mergeFlows([analyze(condition,
        loopPlacement), analyze(body, loopPlacement)]);
      {mayReturn: flow.mayReturn, alwaysReturns: false};
    case TSwitch(subject, cases, fallback):
      ...
      final branchFlows = [
        for (caseValue in cases)
          analyze(caseValue.expr, branchPlacement)
      ];
      final fallbackFlow = fallback == null ? NO_RETURN : analyze(fallback,
        branchPlacement);
      {
        mayReturn: subjectFlow.mayReturn
        || branchFlows.exists(flow -> flow.mayReturn)
        || fallbackFlow.mayReturn,
    ...
    case TTry(body, catches):
      ...
      final catchFlows = [
        for (catchValue in catches)
          analyze(catchValue.expr, protectedPlacement)
      ];
      {
        mayReturn: bodyFlow.mayReturn
        || catchFlows.exists(flow -> flow.mayReturn),
        alwaysReturns: bodyFlow.alwaysReturns
        && catchFlows.foreach(flow -> flow.alwaysReturns)
      };
    case TReturn(result):
      if (result != null) {
...
private function metadataEntries(field: ClassField,
  ...
  switch field.type.follow() {
    case TFun(arguments, result):
      if (components.length == 1 && arguments.length > 1) {
        fail("GTS-REACT-COMPONENT-SIGNATURE-007",
...
private function auditField(type: ClassType, field: ClassField,
    isStatic: Bool): Void {
