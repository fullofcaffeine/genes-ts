...
class JsxPlan {
  ...
  static function isSourceInlineSafeTag(tag: JsxTagIntent): Bool {
    return switch tag {
      ...
      case ComponentTag(expression):
        // Local variables and direct ESM roots are lexical bindings. Reading
        // either cannot invoke a getter or Proxy trap. A dotted jsRequire
        // member remains a property read and is therefore excluded below.
        // Every other static/type/enum field keeps the child temporary because
        // its target runtime may make the lookup observable.
        switch sourceInlineUnwrap(expression).expr {
          case TLocal(_): true;
          case TField(_, FStatic(_, _.get() => field)):
            isDirectJsRequireField(field);
          default: false;
        }
    }
  }

  static function isDirectJsRequireField(field: ClassField): Bool {
    final shape = Dependencies.fieldJsRequireShape(field.meta);
    return shape != null && shape.memberPath.length == 0;
  }
