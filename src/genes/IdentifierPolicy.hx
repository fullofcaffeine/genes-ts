 ...
 * policy byte-for-byte. `isValidModuleBinding` applies those same host/global
 * exclusions to an exact, user-requested top-level binding and adds the
 * stricter ES-module rule for `await`, which remains a legal property name.
 * This prevents a new top-level `Object` or `undefined` declaration from
 * redirecting compiler-generated references that were meant for JavaScript.
 *
