...
function main(): Void {
  ...
  final a = store.add("Buy milk");
  assertEqual(a.id, 1, "first id");
  assertStringEqual(a.title, "Buy milk", "title trimmed");
  assert(genes.js.Equality.strict(a.status, TodoStatus.Active), "new todo is active");
  final b = store.add("Learn Haxe");
  assertEqual(b.id, 2, "second id");
  assertEqual(store.list().length, 2, "list length after add");
  store.toggle(1);
  final afterToggle1 = store.list();
  assertEqual(afterToggle1.length, 2, "list length unchanged after toggle");
  final firstAfterToggle1 = afterToggle1[0];
  assert((firstAfterToggle1 != null), "first todo exists");
  assertEqual(firstAfterToggle1.id, 1, "toggle leaves first todo in list");
  assert(genes.js.Equality.strict(firstAfterToggle1.status, TodoStatus.Done), "toggle sets done");
  store.toggle(1);
  final afterToggle2 = store.list();
  final firstAfterToggle2 = afterToggle2[0];
  assert((firstAfterToggle2 != null), "first todo exists (toggle 2)");
  assert(genes.js.Equality.strict(firstAfterToggle2.status, TodoStatus.Active), "toggle sets active");
