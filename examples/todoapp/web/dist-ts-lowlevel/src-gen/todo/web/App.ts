...
export class App {
	static Component(): JSX.Element {
		...
		let tmp1: JSX.Element = React__genes_jsx.createElement(Link, ({to: "/", style: {"textDecoration": "none"}} satisfies (React__genes_jsx.ComponentPropsWithoutRef<typeof Link> & React__genes_jsx.Attributes & { [K in `data-${string}`]?: string | number | boolean | null | undefined } & { [K in `aria-${string}`]?: string | number | boolean | null | undefined })), "Home");
		...
		let tmp7: JSX.Element = React__genes_jsx.createElement(Routes, null, tmp5, tmp6);
		...
		return React__genes_jsx.createElement(BrowserRouter, null, tmp8);
	}
