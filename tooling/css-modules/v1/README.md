...
the same export-name identity, nor can it resolve `request` relative to
`generatedModule` and compare that result with `hostModulePath`. The tooling
runtime therefore performs those comparisons and rejects duplicate source
paths, duplicate export names, or a declaration path that would not accompany
the emitted CSS import. It accepts any incoming order and sorts both lists
bytewise before hashing and generation, so processor iteration order cannot
