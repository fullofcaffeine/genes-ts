...
Two explicit provider subpaths can create this manifest. Both ways return
`CssModuleExportsManifestV1` and write no files.

`@genes-ts/tooling/css-modules/postcss-modules` exports
`createPostcssModulesManifest`. It runs a closed PostCSS Modules profile and
pins these runtime packages:

- `postcss` 8.5.25.
- `postcss-modules` 9.0.1.
- `postcss-selector-parser` 7.1.4.

Install these optional peer packages before you import this provider:

```bash
npm install --save-exact postcss@8.5.25 postcss-modules@9.0.1 postcss-selector-parser@7.1.4
```

The provider accepts only `generateScopedName`, `scopeBehaviour`,
`exportGlobals`, and `hashPrefix`. Each property must contain an inert data
value. The provider never reads an application configuration file.

The provider records the registry integrity of `postcss-modules`. Its
configuration hash also records the PostCSS and selector-parser versions and
integrities. This identity changes if any accepted data value changes.

The provider resolves relative composition files below the project root. It
rejects package aliases, URL queries, fragments, outside paths, and symbolic
links. The manifest records the SHA-256 hash of each file that the processor
reads.

`@genes-ts/tooling/css-modules/typescript-declaration` exports
`createTypeScriptDeclarationManifest`. It reads `<entry>.d.ts` with pinned
TypeScript 6.0.3. The file must contain one `declare const` with a direct type
literal. Each property must be required, named, `readonly`, and exactly
`string`. The second and final statement must default-export that constant.

Install the declaration provider's optional peer package before use:

```bash
npm install --save-exact typescript@6.0.3
```

The declaration adapter rejects wildcard modules, imports, index signatures,
`Record` types, mapped types, optional or mutable properties, duplicate keys,
and other value types. It records each key's declaration location and includes
both the CSS file and declaration in the input list.

The declaration is the authority for its key list. The adapter does not infer
keys from CSS. The host must make sure that its declaration producer and real
loader agree.

The core `@genes-ts/tooling/css-modules` subpath loads neither parser. Existing
manifest validation and companion generation keep their prior startup path.

## Complete one-shot flow

```text
authored card.module.css
  → pinned provider or closed declaration reports exact keys
  → Genes tooling creates genes.css-module-exports@1 manifest
...
- The pinned provider owns its processor call, relative composition paths,
  export keys, source locations, and input hashes.
- A declaration producer owns the keys in a declaration that the adapter reads.
- The host integration selects the stylesheet, binding, configuration data,
  publication path, and real loader check.
...
- the host can use the pinned provider, adapt one closed declaration, or
  supply an exact manifest from another processor;
