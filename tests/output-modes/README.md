...
- null, real JavaScript `undefined`, optional records, explicit
  ...
  nested Haxe `null` kept distinct from `undefined`, assertion-only type
  dependency retention, and immediate narrowing of `genes.ts.Unknown`;
...
`TempPlan`, `NamePlan`, and `LocalBindingPlan` are the shared pre-emission owners
for these lowering facts. The mutability fixture distinguishes initialized
bindings with no writes from direct reassignment, captured reassignment, and an
initializer-free declaration. The four-oracle evaluation trace protects
ordering, the two-clean-build hash protects deterministic names,
`output-quality.json` rejects new synthetic temporaries (including any temp in
the retained `Main` entry point), and the focused no-`js-es` fixture rejects
colliding inline-expanded TS locals. The printers select syntax only; they do
not allocate iterator/result bindings, discover `TVar` names, or independently
decide `const` versus `let` while writing source.
